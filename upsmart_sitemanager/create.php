<?php
	...
	function upsmart_page_create_nav($n,$sn=null) {
		...
		$out = "<div class='steps'><ol class='steps supersteps'>
			...
			<li".($n==4?" class='active'":"")."><a href='{$base}10'>4. Goals</a></li>
			<li".($n==5?" class='active'":"")."><a href='{$base}11'>5. Financial</a></li>
			<li".($n==6?" class='active'":"")."><a href='{$base}14'>6. Render</a></li>
		...
		if($sn != null) {
			if($n==3) {
				$out .= "<div class='steps'><ol class='steps substeps'>
					...
					<li".($sn==4?" class='active'":"")."><a href='{$base}6'>d. Services</a></li>
					<li".($sn==5?" class='active'":"")."><a href='{$base}7'>e. Products</a></li>
					<li".($sn==6?" class='active'":"")."><a href='{$base}8'>f. Milestones</a></li>
					<li".($sn==7?" class='active'":"")."><a href='{$base}9'>g. Market Overview</a></li>
				</ol></div><br style='clear: both'/>";
			}
			if($n==5) {
				$out .= "<div class='steps'><ol class='steps substeps'>
					<li".($sn==1?" class='active'":"")."><a href='{$base}11'>a. Financial Overview</a></li>
					<li".($sn==2?" class='active'":"")."><a href='{$base}12'>b. Applicant</a></li>
					<li".($sn==3?" class='active'":"")."><a href='{$base}13'>c. Company</a></li>
				</ol></div><br style='clear: both'/>";
			}
		}
...
//		$post->post_title = 'Hosting Signup';
		switch(upsmart_handle_page_info) {
			...
			case '/6': $post = upsmart_page_create_business_services($post); break;
			case '/7': $post = upsmart_page_create_products($post); break;
			case '/8': $post = upsmart_page_create_milestones($post); break;
			case '/9': $post = upsmart_page_create_companydescription($post); break;
			case '/10': $post = upsmart_page_create_goals($post); break;
			case '/11': $post = upsmart_page_create_financial($post); break;
			case '/12': $post = upsmart_page_create_financial_applicant($post); break;
			case '/13'; $post = upsmart_page_create_financial_business($post); break;
			case '/14'; $post = upsmart_page_create_render($post); break;
	...
	function upsmart_page_create_home($post) {
		...
		if(get_current_user_id()) {
			...
			$post->post_content .= <<<EOHTML
				<h4 class= 'subheading'>Before we get going, we'll need you to sign into an account so that you can save your work.<br/>It's free and takes only a moment to create.</h4>
				<div id='twocol'>
					<div>
					<h3>Sign In</h3>
...
EOHTML;
	...
	function upsmart_page_create_business_basic($post) {
		...
		$post->post_content = upsmart_page_create_nav(3,1).'<span class=subheading><h2>Business Information</h2></span>';
		...
		$post->post_content .= <<<EOHTML
		<span class='subheading'>We'd like to know more about {$business['name']}. Just fill out the fields below (everything is required).</span>
		<h3>Basic Info</h3>
EOHTML;
	...
	function upsmart_page_create_business_people($post) {
		...
		$post->post_content = upsmart_page_create_nav(3,2).'<span class=subheading><h2>Business Information</h2></span>';
...
// 			wp_redirect(home_url('create/5'));
		...
		$post->post_content .= <<<EOHTML
		<div class= 'subheading'>We'd like to know more about your company. Just fill out the fields below.
		
		<h3>The {$business['name']} Team</h3>
		Introducing the people behind the idea is important. Feel free to send in snapshots of each person. Putting a face to the product is surprisingly good at making an idea "come to life" in the minds of others.</div>
EOHTML;
	...
	function upsmart_page_create_business_desc($post) {
		...
		$post->post_content .= <<<EOHTML
		We'd like to know more about {$business['name']}. Just fill out the fields below.
		<h3>About the Company</h3>
EOHTML;
		$post->post_content .= upsmart_create_profile_form();
		return $post;
	}
	function upsmart_page_create_business_services($post) {
		global $wpdb;
		$business = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_business WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		...
		if(!empty($_POST)) {
			$result = upsmart_create_business_services_save();
			if($result === false) wp_die("An error has occurred.");
		...
		$post->post_content .= <<<EOHTML
		We'd like to know more about your company. Just fill out the fields below.
		<h3>{$business['name']}'s Services</h3>
EOHTML;
		$post->post_content .= upsmart_create_business_services_form();
		return $post;
	}
	...
	function upsmart_page_create_milestones($post) {
		...
		$post->post_content = upsmart_page_create_nav(3,6).'<h2>Business Information</h2>';
...
EOHTML;
		$post->post_content .= upsmart_create_milestones_form();
		return $post;
	}
	...
	function upsmart_page_create_products($post) {
		...
		$post->post_content = upsmart_page_create_nav(3,5).'<span class=subheading><h2>Business Information</h2></span>';
		...
		<div class=subheading>We'd like to know more about your company. Just fill out the fields below.
		<h3>Products</h3>
		Now it's time to learn more about your company's product(s). Click on the plus button below to add one.</div>
EOHTML;
	...
	function upsmart_page_create_goals($post) {
	global $wpdb;
	$business = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_business WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		...
		$post->post_content = upsmart_page_create_nav(4).'<h2>'.$business['name'].' Goals</h2>';
		
		if(!empty($_POST)) {
			$result = upsmart_create_goals_save();
			if($result === false) wp_die("An error has occurred.");
		...
		$post->post_content .= <<<EOHTML
		<span class='subheading'>A statement of goals and timetables are crucial for execution, especially when you are seeking capital.<br/>Let us know what you've got planned!</span>
EOHTML;
		$post->post_content .= upsmart_create_goals_form();
		return $post;
	}
	function upsmart_page_create_financial($post) {
		...
		$post->post_content = upsmart_page_create_nav(5,1).'<h2>Financial</h2>';
		...
		$post->post_content .= "<div class= 'subheading'>Next, we are going to ask you for some financial information pertaining to both yourself and your company.  While we give you the option of which parts you would like to provide, you will likely need to provide all of the following information in order to get a good deal.  Lenders usually look into the history of both the applicant and that of the company as part of their due diligence.</div>";
	...
	function upsmart_page_create_financial_applicant($post) {
		...
		if(!empty($_POST)) {
			...
			if($result === false) wp_die("An error has occurred.");
			wp_redirect(home_url('create/13'));
 			exit();
		}
		
		
		$post->post_content .= <<<EOHTML
		<div class= 'subheading'>We'd like to know more about your company. Just fill out the fields below.</div>
		<h3>Overview</h3>
EOHTML;
	...
	function upsmart_page_create_financial_business($post) {
		global $wpdb;
		
		upsmart_require_login();
		
		$post->post_content = upsmart_page_create_nav(4,3).'<h2>Personal Financial Statement</h2>';
		
		if(!empty($_POST)) {
			$result = upsmart_create_financial_business_save();
			if($result === false) wp_die("An error has occurred.");
			wp_redirect(home_url('create/14'));
 			exit();
		}
		...
		<h3>About the Company</h3>
EOHTML;
		$post->post_content .= upsmart_create_financial_business_form();
		return $post;
	}
	...
	function upsmart_page_create_render($post) {
		...
		if(!empty($_POST)) {
			...
			wp_redirect(home_url('create/15'));
			exit();
		}
	...
	function upsmart_page_create_companydescription($post) {
		...
		$post->post_content = upsmart_page_create_nav(3,7).'<h2>Business Information</h2>';
		...
		$post->post_content .= <<<EOHTML
		<span class='subheading'>We'd like to know more about your company. Just fill out the fields below.</span>
		<h3>Market Overview</h3>
		{$business['name']} is entering a market made up of competitors, target customers and many unique variables to consider.  Below, tell us a bit about it.
EOHTML;
