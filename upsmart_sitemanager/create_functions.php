...
// 			'no' => ($data['incorporated']!=1)?'checked="checked"':'',
// 		);
		$legal_value=$data['legal_structure'];
		return <<<EOHTML
			<form method='post'>
			<table class='infoentry'>
				...
				<tr>
					<th>How is your company legally structured?</th><td><div id='legal_structure' class='styled-select'><select name="legal_structure" value="Corperation"><option value='{$data['legal_structure']}' selected>$legal_value</option><option value="Sole Proprietorship">Sole Proprietorship</option><option value="Partnership">Partnership</option><option value="Corperation">Corperation</option><option value="S-Corperation">S-Corperation</option><option value="Limited Liability Company">Limited Liability Company</option></select></div></td>
				</tr>
				<tr><th>Web Site URL</th><td class= 'textinput'><input name='project_site' value='{$data['url']}' placeholder='https://www.example.com/'/></td></tr>
			</table>
			<input type='submit' value='Save'/>
			</form>
EOHTML;
	...
	function upsmart_create_business_save() {
		...
		$result = $wpdb->query($wpdb->prepare("REPLACE INTO upsmart_business
		                                      (wordpress_id,name,slogan,incorporated,legal_structure,url)
		                                      VALUES(%d,%s,%s,%d,%s,%s)",
		                                      array(
		                                          get_current_user_id(),
		                                          $_POST['project_name'],
...
// 		echo "booga booga booga";
	...
	<table class= 'wp-editor'>
		...
		<tr><td colspan='2'>Your mission statement is a quick statement of the single, fundamental goal behind what your company is doing.  Companies go through lots of changes as they grow; your mission statement should serve as the anchor that you revisit to determine whether the company's evolution are in line with your vision.</td></tr>
		<tr><td colspan='2'>
EOHTML;
	$settings = array(
	'textarea_rows' => 12,
...
EOHTML;
	...
	$out .= <<<EOHTML
		...
		<tr><td colspan='2'>Let us know your background. Why was your company founded?  By whom?  Is there any particular legacy of note? <br/>Tell us {$business['name']}'s story!</td></tr>
		<tr><td colspan='2'>
EOHTML;
...
// 			echo "<br/> Fields: ".$$field;
	...
	function upsmart_create_milestones_form() {
		...
		$founded = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_founded WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$business = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_business WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		...
		<form method='post'>
			 <div class="subheading">
				  <strong>Please tell us when {$business['name']} was founded.</strong>
			</div>
			<table class= 'milestones' width="350px" border="1">
			<tr>
				<td class= 'milestone'>
...
EOHTML;
			...
			<div class="subheading subheading-top">
				  <strong>Below, share any milestone achievements of {$business['name']}.</strong><br/>These could be first-hires, funding that you've received, revenue achievements, or anything else.
			</div>
			<script type="text/javascript" src="https://upsmart.com/wp-content/plugins/upsmart_sitemanager/js/create_business_milestone.js"></script>
			<div id='milestones'>
			<div id='addRow'><input type="button" value="Add Milestone" onclick="addRow('dataTable')" /></div>
...
EOHTML;
	...
	function upsmart_create_milestones_save() {
		...
		echo "<br/>Day: ".$_POST["foundday"];
		echo "<br/>Month: ".$_POST["foundmonth"];
		echo "<br/>Year: ".$_POST["foundyear"]."<br/>";
		
		$result_founded = $wpdb->query($wpdb->prepare("REPLACE INTO upsmart_founded
		                                      (wordpress_id,month,day,year)
		                                      VALUES(%d,%s,%d,%d)",
		                                      array(
							  get_current_user_id(),
							  $_POST["foundmonth"],
...
// 		echo "hello world! <br />";
// 		foreach($data as $p) {
// 			echo "Fname: ".$p->fname."<br>/>";
// 			echo "Lname: ".$p->lname."<br>/>";
// 			echo "Title: ".$p->title."<br>/>";
// 			echo "Bio: ".$p->bio."<br>/>";
// // 			echo $p->photo."<br>/>";
// 			echo "Owner: ".$p->owner."<br>/>";
// 			echo "Own_percentage: ".$p->ownership_percentage."<br>/>";
// 		}

		foreach($data as $p) {
			echo "<br/>".$p->fname." ".$p->lname.": ".$p->owner;
			if($p == null) continue;
			if (!isset($p->owner)) {
			$p->owner = 0;
			echo $p->owner."<br/>";
			...
			$result = $wpdb->query($wpdb->prepare("INSERT INTO upsmart_people
							(wordpress_id,fname,lname,title,bio,photo,owner,percent,founder,edu,skills,prof,awards,community,years,compensation)
...
EOHTML;
	...
	function upsmart_create_goals_form(){
		global $wpdb;
		$threemonths = $wpdb->get_results($wpdb->prepare("SELECT * FROM upsmart_goals WHERE wordpress_id=%d AND timetable='threemonths'",get_current_user_id()),ARRAY_A);
		$nextyear = $wpdb->get_results($wpdb->prepare("SELECT * FROM upsmart_goals WHERE wordpress_id=%d AND timetable='nextyear'",get_current_user_id()),ARRAY_A);
		$twoyears = $wpdb->get_results($wpdb->prepare("SELECT * FROM upsmart_goals WHERE wordpress_id=%d AND timetable='twoyears'",get_current_user_id()),ARRAY_A);
		$business = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_business WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$out = <<<EOHTML
		<script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/jquery/1.5/jquery.min.js"></script>
		<form method='post' id='goals'>
		
		<script type="text/javascript" src="https://upsmart.com/wp-content/plugins/upsmart_sitemanager/js/create_business_milestone.js"></script>
EOHTML;
		$timetables = array('threemonths','nextyear','twoyears');
		foreach($timetables as $timetable)
// 		echo "<br/>Timetable: ".$timetable."<br/>";
		if(empty($$timetable)){
			array_push($$timetable, get_current_user_id());
		}
		$out .= <<<EOHTML
		<div id='goals'>
			
			<div class='goalhead'><h4>What does {$business['name']} aim to achieve within the next three months?</h4>
			<div class='addRow'><input type="button" value="Add Goal" onclick="addRow('threemonths')" /></div></div>
		<table id="threemonths" class= 'milestones' width="350px" border="1">
EOHTML;
		$count = 0;
		foreach($threemonths as $goal){
		$out .= <<<EOHTML
			<tr>
				<td class= 'deleteRow'><input type="button" name="Delete" onClick="newDelete(this);"/> </td>
...
EOHTML;
	...
	$out .= <<<EOHTML
		...
		<div class='goalhead'>
			  <h4>What does {$business['name']} aim to achieve within the next year?</h4>
		<div class='addRow'><input type="button" value="Add Goal" onclick="addRow('nextyear')" /></div>
		</div>
		<table id="nextyear" class= 'milestones' width="350px" border="1">
EOHTML;
		$count = 0;
		foreach($nextyear as $goal){
		$out .= <<<EOHTML
			<tr>
				<td class= 'deleteRow'><input type="button" name="Delete" onClick="newDelete(this);"/> </td>
				<td><input type="text" name="txt[]" value='{$goal['description']}'/><input type="hidden" name="type[]" value='nextyear'></td>
			</tr>
EOHTML;
		} /* END FOR EACH LOOP ON MILESTONES */
	...
	$out .= <<<EOHTML
		...
		<div class='goalhead'>
			<h4>What does {$business['name']} aim to achieve within the next year?</h4>
			<div class='addRow'><input type="button" value="Add Goal" onclick="addRow('twoyears')" /></div>
		</div>
		<table id="twoyears" class= 'milestones' width="350px" border="1">
EOHTML;
		$count = 0;
		foreach($twoyears as $goal){
		$out .= <<<EOHTML
			<tr>
				<td class= 'deleteRow'><input type="button" name="Delete" onClick="newDelete(this);"/> </td>
				<td><input type="text" name="txt[]" value='{$goal['description']}'/><input type="hidden" name="type[]" value='twoyears'></td>
			</tr>
EOHTML;
		} /* END FOR EACH LOOP ON MILESTONES */
...
EOHTML;
	...
	function upsmart_create_goals_save(){
			global $wpdb;
		$i = 0;
		$wpid = get_current_user_id();
		mysql_query("DELETE FROM upsmart_goals WHERE wordpress_id=".$wpid."") or die(mysql_error());

		foreach ($_POST["txt"] as $goal){

			$description = $_POST["txt"][$i];
			$type=$_POST["type"][$i];
			echo get_current_user_id();
			echo $description;
			echo "<br/>".$type."<br/>";
			$result = $wpdb->query($wpdb->prepare("REPLACE INTO upsmart_goals
		                                      (wordpress_id,description,timetable)
		                                      VALUES(%d,%s,%s)",
		                                      array(
		                                          get_current_user_id(),
		                                          $description,
...
EOHTML;
	...
	function upsmart_create_business_services_form(){
		global $wpdb;
		$data = $wpdb->get_results($wpdb->prepare("SELECT * FROM upsmart_services WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$months = array('January','February','March','April','May','June','July ','August','September','October','November','December');
		if(empty($$timetable)){
			array_push($data, get_current_user_id());
		}
		$out = <<<EOHTML
		<script src="js/modernizr-1.7.min.js"></script>
		<script type="text/javascript" src="https://ajax.googleapis.com/ajax/libs/jquery/1.5/jquery.min.js"></script>
		<form method='post'>
			<div class="subheading">
			Below, give a 160 character description of each service your company offers.
			</div>
			<script type="text/javascript" src="https://upsmart.com/wp-content/plugins/upsmart_sitemanager/js/create_business_services.js"></script>
			<div id='services'>
			<div id='addRow'><input type="button" value="Add Service" onclick="addRow('dataTable')" /></div>
			<table id= 'dataTable' class= 'services' width="350px" border="1">
EOHTML;
		foreach($data as $service){
		$out .= <<<EOHTML
			<tr>
				<td class= 'deleteRow'><input type="button" name="Delete" onClick="newDelete(this);"/> </td>
				<td><input type="text" name="txt[]" value='{$service['description']}'/></td>
			</tr>
EOHTML;
		} /* END FOR EACH LOOP ON MILESTONES */
...
EOHTML;
	...
	function upsmart_create_business_services_save(){
		global $wpdb;
		$i = 0;
		$wpid = get_current_user_id();
		mysql_query("DELETE FROM upsmart_services WHERE wordpress_id=".$wpid."") or die(mysql_error());
		foreach ($_POST["txt"] as $txt_value){
			echo "<br/>it is: ".$_POST["txt"][$i]."<br/>";
			$txt = $_POST["txt"][$i];
			
			$result = $wpdb->query($wpdb->prepare("REPLACE INTO upsmart_services
	...
	function upsmart_create_financial_applicant_form(){
		...
		$liabilities = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_personal_financial_liabilities WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
			...
			return <<<EOHTML
		<script type="text/javascript" src="https://upsmart.com/wp-content/plugins/upsmart_sitemanager/js/create_finance.js"></script>
...
EOHTML;
	...
	function upsmart_create_financial_applicant_save() {
		...
		if($result_assets === false) return false;
		if($result_liabilities === false) return false;
		return true;
	}
...
EOHTML;
	...
	function upsmart_create_render_form() {
		global $wpdb;
...
// 		define('FPDF_FONTPATH','fpdf/');
		...
		$contact = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_contact WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$contact['phone'] = explode('-',$contact['phone']);
		$business = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_business WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$profile = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_profile WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$milestones = $wpdb->get_results($wpdb->prepare("SELECT * FROM upsmart_milestones WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$people = $wpdb->get_results($wpdb->prepare("SELECT * FROM upsmart_people WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$products = $wpdb->get_results($wpdb->prepare("SELECT * FROM upsmart_products WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$milestones = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_description WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$personal_assets = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_personal_financial_assets WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$personal_liabilities = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_personal_financial_liabilities WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$business_assets = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_business_financial_assets WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);
		$business_liabilities = $wpdb->get_row($wpdb->prepare("SELECT * FROM upsmart_business_financial_liabilities WHERE wordpress_id=%d",get_current_user_id()),ARRAY_A);

// 		var_dump($personal_liabilities);
// 		echo $mediadir;
// 		echo $logo;
		...
		function Header()
		{
		    $mediadir = WP_CONTENT_DIR.'/uploads/upsmart/'.get_current_user_id().'/';
		    $logos = $mediadir."logo.png";
		    // Logo
		    $this->Image($logos,10,0,30);
		    // Arial bold 15
		...
		function WriteHTML($example)
		{
		    // HTML parser
		    $example = str_replace("\n",' ',$example);
		    $a = preg_split('/<(.*)>/U',$example,-1,PREG_SPLIT_DELIM_CAPTURE);
		    foreach($a as $i=>$e)
		    {
			if($i%2==0)
			{
		...
		$pdf->Cell(0,18,'Executive Summary',0,1);
		$pdf->SetFont('Times','I',12);
		$pdf->Cell(0,18,$profile['mission'],0,1);
		$pdf->SetFont('Times','I',12);
		$pdf->SetFont('Times','',12);
		$pdf->Cell(0,14, $profile['about'],0,1);
		$example = 'You can now easily print text mixing different styles: <b>bold</b>, <i>italic</i>,
		<u>underlined</u>, or <b><i><u>all at once</u></i></b>!<br><br>You can also insert links on
		text, such as <a href="http://www.fpdf.org">www.fpdf.org</a>, or on an image: click on the logo.';
		$pdf->WriteHTML(nl2br($profile['about']));
		$pdf->SetFont('Times','B',18);
		$pdf->Cell(0,18,'Business Focus',0,1);
		$pdf->SetFont('Times','B',12);
		$pdf->Cell(0,12,'Address:',0,1);
		$pdf->SetFont('Times','',12);
		$pdf->WriteHTML(nl2br($business['name']."<br>".$contact['street']."<br>".$contact['city']." ".$contact['zip']));	
		$pdf->SetFont('Times','B',12);
		$pdf->Cell(0,12,'Number of Years in Business:',0,1);
// 		$pdf->SetFont('Times','',12);
// 		$pdf->WriteHTML(nl2br($business['
		$pdf->SetFont('Times','B',12);
		$pdf->Cell(0,12,'Legal Structure:',0,1);
		$pdf->SetFont('Times','',12);
		$pdf->WriteHTML(nl2br($business['legal_structure']));

		$pdf->AddPage();
		$pdf->SetFont('Times','B',18);
// 		foreach ($people as $member){
// 			$pdf->Cell(0,12, $people['fname'],0,1);
// 		}
		
		$pdf->AddPage();
		$pdf->SetFont('Times','B',18);
		$pdf->Cell(0,18,'Services',0,1);
		$pdf->SetFont('Times','B',18);
		$pdf->Cell(0,18,'Products',0,1);
